            ...
            <header class="glass-panel sticky top-0 z-40 px-4 py-6 border-b border-white/20">
                <div class="max-w-5xl mx-auto flex justify-between items-center">
                    <div class="flex items-center gap-4">
                        <button onclick="window.history.back()" class="p-3 glass-panel rounded-2xl text-gray-500 hover:text-blue-600 transition-colors shadow-sm">
            ...
            content.innerHTML = `
                <div class="bg-white p-10 rounded-[50px] shadow-2xl shadow-blue-50/50 border border-white mb-10 relative overflow-hidden">
                    ...
                    <div class="relative z-10">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                            <div class="bg-blue-premium p-8 rounded-[32px] text-center shadow-xl shadow-blue-100">
                                <p class="text-[10px] font-black text-blue-100 uppercase tracking-widest mb-2">Total Packets</p>
                        ...
                        <div class="mt-10 flex justify-center">
                            <button id="grade-all-btn" class="bg-blue-premium text-white px-10 py-5 rounded-2xl font-black uppercase text-xs tracking-[0.2em] shadow-xl hover:shadow-2xl hover:-translate-y-0.5 active:scale-95 transition-all">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="divide-y divide-gray-50 text-sm">
                                ${submissions.length === 0 ? '<tr><td colspan="4" class="p-20 text-center font-black text-xs text-gray-300 uppercase tracking-[0.3em]">No telemetry detected in this sector</td></tr>' : ''}
                                ${submissions.map(s => {
                ...
                return `
                                        <tr class="hover:bg-blue-50/30 transition-colors group">
                                            <td class="p-8">
                                                <div class="flex items-center gap-4">
                                                    <div class="w-12 h-12 bg-gray-100 rounded-2xl flex items-center justify-center text-gray-500 font-black text-lg group-hover:bg-blue-premium group-hover:text-white transition-all">
                                                        ${s.studentName?.[0] || '?'}
                                                    </div>
                                                    <div>
                                                        <p class="font-black text-gray-900 uppercase tracking-tight">${s.studentName || 'Unknown Asset'}</p>
                                                        <p class="text-[10px] font-bold text-gray-500 tracking-wider">${s.studentEmail}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="p-8 text-[11px] font-black text-gray-500 uppercase tracking-tight">
                                            ...
                                            <td class="p-8 text-right font-black text-xl text-gray-900 tracking-tighter">
                                                ${isGraded ? `<span class="text-blue-600">${s.score}</span> <span class="text-gray-200 text-sm font-bold">/ ${s.totalPoints || '?'}</span>` : '<span class="text-gray-200">--</span>'}
                                            </td>
                                        </tr>
                                    `;
            }).join('')}
