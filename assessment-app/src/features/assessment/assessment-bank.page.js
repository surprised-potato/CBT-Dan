    ...
    const renderList = () => {
        ...
        if (filtered.length === 0) {
            listContainer.innerHTML = `
                <div class="text-center py-24 glass-panel rounded-[50px] border-2 border-dashed border-white">
                    <p class="text-gray-400 font-black uppercase tracking-widest text-xs opacity-50 mb-6">No operational modules found</p>
        ...
        listContainer.innerHTML = filtered.map(a => {
            ...
            const statusBadge = isActive
                ...
                : `<span class="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest bg-gray-100 text-gray-400 border border-gray-200">
                     Draft
                   </span>`;
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-4 mb-4">
                            ${statusBadge}
                            <span class="text-[10px] font-black text-gray-300 uppercase tracking-[0.2em]">${new Date(a.createdAt).toLocaleDateString(undefined, { month: 'long', day: 'numeric', year: 'numeric' })}</span>
                        </div>
                        ...
                        <p class="text-[10px] text-gray-400 font-bold uppercase tracking-[0.2em] mt-2 flex items-center gap-2">
                    ...
                    <div class="flex items-center gap-4 mt-4 md:mt-0">
                        <div class="flex gap-2">
                            <button onclick="window.editAsTitle('${a.id}', '${a.title.replace(/'/g, "\\'")}')" class="w-12 h-12 glass-panel rounded-2xl text-gray-400 hover:text-blue-600 transition-all flex items-center justify-center shadow-sm" title="Modify Registry">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                            </button>
                            <button onclick="window.delAs('${a.id}')" class="w-12 h-12 glass-panel rounded-2xl text-gray-400 hover:text-red-500 transition-all flex items-center justify-center shadow-sm" title="Purge Records">
