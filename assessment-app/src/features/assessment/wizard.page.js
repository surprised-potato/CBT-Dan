...
export const WizardPage = async () => {
    ...
    app.innerHTML = `
        <div class="px-4 py-12 max-w-3xl mx-auto pb-32">
            <header class="mb-12">
                <button onclick="location.hash='#assessment-bank'" class="p-3 glass-panel rounded-2xl text-gray-500 hover:text-blue-600 transition-all shadow-sm mb-6">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                </button>
                <h1 class="text-4xl font-black text-gray-900 tracking-tight uppercase">Module Architect</h1>
                <p class="text-xs font-black text-gray-500 uppercase tracking-[0.3em] mt-2">Configure multi-sector assessment parameters</p>
            </header>

            <div class="space-y-8">
                <!-- Main Config -->
                <div class="bg-white p-10 rounded-[50px] shadow-2xl shadow-blue-50/50 border border-white space-y-6 relative overflow-hidden">
                    <div class="absolute top-0 right-0 w-32 h-32 bg-blue-50 rounded-full -mr-16 -mt-16 blur-3xl opacity-50"></div>
                    
                    <div class="relative z-10">
                        <label class="text-[10px] font-black text-gray-500 uppercase tracking-[0.3em] mb-4 block">Operational Identification</label>
                    ...
                    <div class="flex flex-col gap-2 relative z-10">
                        <label class="text-[10px] font-black text-gray-500 uppercase tracking-[0.3em]">Deployment Sector (Optional)</label>
                        <select id="assigned-class" class="p-5 bg-gray-50 border border-gray-100 rounded-2xl focus:bg-white focus:border-blue-500 outline-none transition-all font-bold text-gray-600 appearance-none shadow-inner">
                ...
                <!-- Add Section Button -->
                <button id="add-section-btn" class="w-full py-8 glass-panel border-2 border-dashed border-gray-200 rounded-[40px] text-gray-500 font-extrabold uppercase tracking-widest hover:border-blue-300 hover:text-blue-500 hover:bg-white transition-all active:scale-[0.98] shadow-sm">
                ...
                <div class="fixed bottom-0 left-0 right-0 p-6 bg-white/60 backdrop-blur-xl border-t border-white/20 flex justify-center z-50">
                    <div class="w-full max-w-3xl">
                        <button id="final-submit-btn" class="w-full bg-blue-premium text-white p-6 rounded-3xl font-black uppercase text-sm tracking-[0.3em] shadow-2xl shadow-blue-200/50 hover:shadow-blue-300 hover:-translate-y-1 active:scale-[0.98] transition-all">Execute Generation Protocol</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    const renderSections = () => {
        ...
        container.innerHTML = sections.map((s, i) => `
            <div class="bg-white p-10 rounded-[50px] border border-white shadow-2xl shadow-blue-50/50 relative group animate-in fade-in slide-in-from-bottom-8 duration-500 overflow-hidden">
                <div class="absolute top-0 right-0 w-48 h-full bg-gradient-to-l from-blue-50/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
                ...
                <div class="relative z-10">
                    <div class="flex items-center gap-4 mb-8">
                        <div class="w-10 h-1bg-blue-premium rounded-full"></div>
                        <p class="text-[10px] font-black text-blue-600 uppercase tracking-[0.4em]">Sector Archive ${i + 1}</p>
                    </div>
                    
                    <div class="space-y-8">
                        <div class="flex flex-col gap-2">
                            <label class="text-[10px] font-black text-gray-500 uppercase tracking-[0.3em]">Sector Designation</label>
                            <input type="text" value="${s.title}" data-field="title" data-idx="${i}" placeholder="e.g. PART I: QUANTITATIVE ANALYSIS" class="sec-input p-5 bg-gray-50 border border-gray-100 rounded-2xl focus:bg-white focus:border-blue-500 outline-none transition-all font-black text-gray-700 uppercase placeholder:opacity-30">
                        </div>

                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div class="flex flex-col gap-2">
                                <label class="text-[10px] font-black text-gray-500 uppercase tracking-[0.3em]">Knowledge Domain</label>
                                <select data-field="course" data-idx="${i}" class="sec-input p-5 bg-gray-50 border border-gray-100 rounded-2xl focus:bg-white focus:border-blue-500 outline-none transition-all font-bold text-gray-600 appearance-none shadow-inner">
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <select data-field="type" data-idx="${i}" class="sec-input p-5 bg-gray-50 border border-gray-100 rounded-2xl focus:bg-white focus:border-blue-500 outline-none transition-all font-bold text-gray-600 appearance-none shadow-inner">
                        ...
                        <div class="flex flex-col gap-4">
                            <label class="text-[10px] font-black text-gray-500 uppercase tracking-[0.3em]">Vector Selection (Multi-Track)</label>
            ...
            return `
                                            <label class="cursor-pointer">
                                                ...
                                                <div class="topic-chip px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ${isChecked ? 'bg-blue-premium text-white border-blue-400 shadow-lg shadow-blue-100' : 'bg-white text-gray-500 border-gray-100 hover:border-blue-200'}">
                                                    ${t}
                                                </div>
                                            </label>
                                        `;
                        ...
                        <div class="p-8 bg-blue-50/30 rounded-[36px] border border-blue-100/30 shadow-inner">
                            <div class="flex justify-between items-center mb-6">
                                <label class="text-[10px] font-black text-blue-600 uppercase tracking-[0.3em]">Complexity Distribution</label>
                                <div class="flex items-center gap-3 bg-white px-4 py-2 rounded-xl border border-blue-100 shadow-sm">
                                    <span class="text-[10px] font-black text-gray-500 uppercase">Val/Q:</span>
                                    <input type="number" value="${s.pointsPerQuestion}" min="1" data-field="pointsPerQuestion" data-idx="${i}" class="sec-input w-8 text-center text-xs font-black text-blue-600 outline-none">
                                </div>
                            </div>
        ...
        container.querySelectorAll('.topic-check').forEach(el => {
            el.onchange = (e) => {
                ...
                if (e.target.checked) {
                    ...
                    chip.classList.add('bg-blue-premium', 'text-white', 'border-blue-400', 'shadow-lg', 'shadow-blue-100');
                } else {
                    chip.classList.add('bg-white', 'text-gray-400', 'border-gray-100');
                    chip.classList.remove('bg-blue-premium', 'text-white', 'border-blue-400', 'shadow-lg', 'shadow-blue-100');
                }
            };
        });
