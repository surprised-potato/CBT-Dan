...
export const EditorPage = async () => {
    ...
    app.innerHTML = `
        <div class="px-4 py-12 max-w-3xl mx-auto pb-32 relative min-h-screen">
            <header class="mb-12">
                ...
                <p id="page-desc" class="text-[10px] font-black text-gray-600 uppercase tracking-[0.3em] mt-2">Author operational items for registry bank</p>
            </header>
                ...
                <div id="dynamic-content" class="bg-white p-10 rounded-[50px] border border-white shadow-2xl shadow-blue-50/50 relative overflow-hidden">
                    ...
                    <div id="section-mcq" class="space-y-6 hidden relative z-10">
                        <div class="flex justify-between items-center px-2">
                            <label class="text-[10px] font-black text-gray-700 uppercase tracking-[0.3em]">Alternative Responses</label>
                    ...
                    <div id="section-id" class="space-y-6 hidden relative z-10">
                        <label class="text-[10px] font-black text-gray-700 uppercase tracking-[0.3em] px-2 block">Accepted Data Variants</label>
                        <div class="flex gap-4">
                            <input id="id-input" type="text" class="flex-1 p-5 bg-gray-50 border border-gray-100 rounded-2xl focus:bg-white focus:border-blue-500 outline-none transition-all font-black text-gray-900 uppercase shadow-inner" placeholder="ADD VARIANT...">
    ...
    const renderChoices = () => {
        els.choicesList.innerHTML = state.choices.map((c, i) => {
            const isCorrect = state.correctMCQ === c.id;
            return `
            <div class="flex items-center gap-4 group/row animate-in fade-in slide-in-from-left-4 duration-300 p-2 rounded-[28px] transition-all ${isCorrect ? 'bg-blue-50/50 border border-blue-100 shadow-inner' : 'border border-transparent hover:bg-gray-50/50'}">
                <label class="relative flex-shrink-0 cursor-pointer group/choice">
                    <input type="radio" name="mcq-correct" value="${c.id}" ${isCorrect ? 'checked' : ''} class="peer hidden">
                ...
                <div class="flex-1 flex flex-col gap-1">
                    ${isCorrect ? `<span class="text-[8px] font-black text-blue-600 uppercase tracking-[0.3em] ml-2 mb-1 animate-pulse">CORRECT OPTION</span>` : ''}
                    <input type="text" value="${c.text}" data-idx="${i}" class="choice-input w-full p-5 bg-gray-50/50 border border-gray-100 rounded-2xl text-sm font-black uppercase tracking-tight transition-all focus:bg-white focus:border-blue-500 outline-none text-gray-900 placeholder:opacity-30" placeholder="DATA ALT ${String.fromCharCode(65 + i)}...">
                </div>
                <button type="button" data-idx="${i}" class="remove-choice w-12 h-12 flex items-center justify-center text-gray-300 hover:text-red-500 font-black text-2xl transition-colors opacity-0 group-hover/row:opacity-100">×</button>
            </div>
        `}).join('');
        els.choicesList.querySelectorAll('input[type="radio"]').forEach(r => {
            r.onchange = () => {
                state.correctMCQ = r.value;
                renderChoices(); // Re-render to update row highlighting and labels
            };
        });
    ...
    const renderIdTags = () => {
        els.idList.innerHTML = state.acceptedAnswers.map((ans, i) => `
            <span class="inline-flex items-center px-6 py-3 rounded-2xl text-[10px] font-black bg-white text-blue-600 border border-blue-100 shadow-xl shadow-blue-50/50 animate-in zoom-in uppercase tracking-widest group/tag">
                ${ans}
                <button type="button" data-idx="${i}" class="rm-tag ml-4 text-gray-300 hover:text-red-500 transition-colors font-black text-lg">×</button>
            </span>
        `).join('');
    ...
    const init = async () => {
        try {
            ...
            if (state.questionId) {
                console.log(`Editing existing question: ${state.questionId}`);
                ...
                if (q) {
                    ...
                    els.type.value = q.type || 'MCQ';
                    els.difficulty.value = q.difficulty || 'EASY';
                    els.points.value = q.points || 1;

                    if (state.quill) state.quill.root.innerHTML = q.text || '';
            ...
            console.error("Editor Protocol Failure:", err);
            alert("Protocol Failure: Telemetry sync error.\n\n" + err.message);
            location.hash = '#bank';
        }
    };
    ...
    els.form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            els.status.textContent = "❌ COMMIT PROTOCOL FAILURE: " + err.message;
